...
- ### 게시판 목록 조회
  ...
  - 조회 시 검색을 통해 원하는 게시판을 조회가능하다.
  - 검색은 제목으로 검색 가능하고 검색어가 없다면 모든 게시판이 조회가능하다.
  - 10개씩 페이징되서 목록이 출력된다.
- ### 게시글 작성
  - 로그인한 유저만 글 작성이 가능하다.
...
- ### 내 정보 보기
  - 로그인한 사용자만 정보보기가 가능하다.
  - 응답 값으로는 이메일, 이름, 생년월일, 휴대폰 번를 출력한다.
- ### 내 정보 수정하기
  - 로그인한 사용자만 정보 수정하가 가능하다.
  - 수정 값으로는 이름, 생년월일, 휴대폰 번호를 변경한다.
  - 변경일은 지금시간으로 갱신한다.
